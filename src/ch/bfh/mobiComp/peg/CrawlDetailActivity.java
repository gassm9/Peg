...
//		this.setTitle(getIntent().getStringExtra("CRAWLNAME"));
	...
	private void loadPubsForCrawl(String crawlId) {
		MobileServiceTable<PubCrawlItem> pubCrawlTable = mClient
				.getTable(PubCrawlItem.class);
				...
				.execute(new TableQueryCallback<PubCrawlItem>() {
					...
					public void onCompleted(List<PubCrawlItem> result,
						...
						for (PubCrawlItem pubCrawl : result) {
							...
							// Load Pubs
							MobileServiceTable<PubItem> pubTable = mClient
									.getTable(PubItem.class);
							pubTable.where().field("id")
									.eq(pubCrawl.getPubId())
									.execute(new TableQueryCallback<PubItem>() {
										@Override
										...
										private void setUpMap(int count) {
											...
											// LatLng firstpub = new
											// LatLng(Double.valueOf(pubAdapter.getItem(0).getLatitude()),
											// Double.valueOf(pubAdapter.getItem(0).getLongitude()));
											LatLng firstpub = new LatLng(Double
													.valueOf(pubAdapter
															.getItem(0)
															.getLongitude()),
													Double.valueOf(pubAdapter
															.getItem(0)
															.getLatitude()));
											...
											for (int i = 0; i < pubAdapter
													.getCount(); i++) {
												PubItem pub = pubAdapter
														.getItem(i);
												// TODO lat, long switch
												// LatLng pubPos = new
												// LatLng(Double.valueOf(pub.getLatitude()),
												// Double.valueOf(pub.getLongitude()));
												LatLng pubPos = new LatLng(
														Double.valueOf(pub
																.getLongitude()),
														Double.valueOf(pub
																.getLatitude()));
											...
											mapFragment.getMap().addPolyline(
													line);
